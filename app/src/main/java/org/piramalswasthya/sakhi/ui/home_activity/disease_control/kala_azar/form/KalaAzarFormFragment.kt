...
@AndroidEntryPoint
class KalaAzarFormFragment : Fragment() {
    private var _binding: FragmentMaleriaFormBinding? = null

    private val viewModel: KalaAzarFormViewModel by viewModels()
            ...
            when (it) {
                KalaAzarFormViewModel.State.SAVE_SUCCESS -> {
                    if (viewModel.isDeath) {
                        ...
                        setMessage(R.string.kala_submitted)
    ...
    override fun onStart() {
        ...
        activity?.let {
            (it as HomeActivity).updateActionBar(
                R.drawable.ic__ncd,
                getString(R.string.kala_screening_form)
            )
        }
    }
