        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                try {
                    ...
                    leprosyScreenCache.syncState = SyncState.UNSYNCED
        ...
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                try {
                    _state.postValue(State.SAVING)
                    dataset.mapValues(leprosyScreenCache, 1)
                    leprosyScreenCache.syncState = SyncState.DRAFT
                    leprosyRepo.saveLeprosyScreening(leprosyScreenCache)
                    _state.postValue(State.DRAFT_SAVED)
                } catch (e: Exception) {
                    Timber.d("saving leprosy screening draft data failed!!")
                    _state.postValue(State.SAVE_FAILED)
                }
            }
        }
