...
class BenIfaFormRepository @Inject constructor(
    ...
    private val jsonResponseDao = db.formResponseJsonDaoBenIfa()
    ...
    suspend fun canAddNewVisit(benId: Long): Boolean {
        ...
        // Parse all stored visit dates
        val anyThisMonth = jsonList.any { json ->
            try {
                val obj = JSONObject(json)
                val dateString = obj.optString("visitDate", "")
                val date = dateFormat.parse(dateString)
                if (date != null) {
                    val visitCal = Calendar.getInstance().apply { time = date }
                    visitCal.get(Calendar.MONTH) == currentMonth &&
                            visitCal.get(Calendar.YEAR) == currentYear
                } else false
            } catch (e: Exception) {
                false
            }
    ...
    suspend fun getBottleList(benId: Long, formId: String): List<BottleItem> {
        ...
        return result
            .sortedByDescending { item ->
                try {
                    dateFormat.parse(item.dateOfProvision)?.time ?: 0L
                } catch (e: Exception) {
                    0L
                }
            }
    ...
    suspend fun insertOrUpdateFormResponse(entity: BenIfaFormResponseJsonEntity) {
    ...
    suspend fun insertFormResponse(entity: BenIfaFormResponseJsonEntity) =
        jsonResponseDao.insertFormResponse(entity)

    suspend fun loadFormResponseJson(benId: Long, visitDate: String): String? =
        jsonResponseDao.getFormResponse(benId,visitDate)?.formDataJson

    suspend fun getUnsyncedForms(formName: String): List<BenIfaFormResponseJsonEntity> =
        jsonResponseDao.getUnsyncedForms(formName)

    suspend fun syncFormToServer(userName: String,formName: String, form: BenIfaFormResponseJsonEntity): Boolean {
        return try {
