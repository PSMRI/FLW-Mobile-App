        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (message.contains("UIDAI Error code : 400 : OTP validation failed")){
                        return@withContext NetworkResult.Error(-6, "No match for this face, Please proceed with registered person")
                    }

                    if (message.contains("reached the maximum limit")){
                        return@withContext NetworkResult.Error(1227, "ABHA Creation Limit Reached! Please try again late.")
                    }
