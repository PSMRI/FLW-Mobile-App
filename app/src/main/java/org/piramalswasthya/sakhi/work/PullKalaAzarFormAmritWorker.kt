...
@HiltWorker
class PullKalaAzarFormAmritWorker @AssistedInject constructor(
    ...
    companion object {
        const val name = "PullkalaAzarFromAmritWorker"
        ...
        return try {
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    val result1 =
                        awaitAll(
                            async { getKalaAzarScreeningDetails() },
                    ...
                    return@withContext Result.failure(workDataOf("worker_name" to "PullKalaAzarFormAmritWorker", "error" to "Pull operation returned incomplete results"))
                } catch (e: SQLiteConstraintException) {
                    Timber.e("exception $e raised ${e.message} with stacktrace : ${e.stackTrace}")
                    return@withContext Result.failure(workDataOf("worker_name" to "PullKalaAzarFormAmritWorker", "error" to "SQLite constraint: ${e.message}"))
                }
            ...
            Result.failure(workDataOf("worker_name" to "PullKalaAzarFormAmritWorker", "error" to (e.message ?: "Unknown error")))
        }
    }
    ...
    private suspend fun getKalaAzarScreeningDetails(): Boolean {
