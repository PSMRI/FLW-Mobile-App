...
@HiltWorker
class PullFilariaFromAmritWorker @AssistedInject constructor(
    ...
    companion object {
        const val name = "PullFilariaFromAmritWorker"
        ...
        return try {
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    val result1 =
                        awaitAll(
                            async { getFilariaScreeningDetails() },
                    ...
                    return@withContext Result.failure(workDataOf("worker_name" to "PullFilariaFromAmritWorker", "error" to "Pull operation returned incomplete results"))
                } catch (e: SQLiteConstraintException) {
                    Timber.e("exception $e raised ${e.message} with stacktrace : ${e.stackTrace}")
                    return@withContext Result.failure(workDataOf("worker_name" to "PullFilariaFromAmritWorker", "error" to "SQLite constraint: ${e.message}"))
                }
            ...
            Result.failure(workDataOf("worker_name" to "PullFilariaFromAmritWorker", "error" to (e.message ?: "Unknown error")))
        }
    }
    ...
    private suspend fun getFilariaScreeningDetails(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val res = filariaRepo.getFilariaScreeningDetailsFromServer()
