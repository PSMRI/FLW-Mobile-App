...
class PushAdolescentAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushAdolescentAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushAdolescentAmritWorker(context, params, adolescentHealthRepo, preferenceDao)
    }
    ...
    fun doSyncWork_returnsSuccess_whenRepoReturnsTrue() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } returns true
        assertTrue(worker().doWork() is ListenableWorker.Result.Success)
    }

    @Test
    fun doSyncWork_reportsSyncFalse_whenRepoReturnsFalse() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } returns false
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } throws java.net.SocketTimeoutException("timeout")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } throws java.io.IOException("offline")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_returnsFailureWithMessage_whenGenericExceptionThrown() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } throws IllegalStateException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionHasNoMessage() = runTest {
        coEvery { adolescentHealthRepo.pushUnSyncedRecords() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
