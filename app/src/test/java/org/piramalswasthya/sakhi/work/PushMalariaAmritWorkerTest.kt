...
class PushMalariaAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushMalariaAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushMalariaAmritWorker(context, params, malariaRepo, preferenceDao)
    }
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { malariaRepo.pushUnSyncedRecords() } throws java.net.SocketTimeoutException("timeout")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { malariaRepo.pushUnSyncedRecords() } throws java.io.IOException("offline")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_returnsFailureWithMessage_whenGenericExceptionThrown() = runTest {
        coEvery { malariaRepo.pushUnSyncedRecords() } throws IllegalStateException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionHasNoMessage() = runTest {
        coEvery { malariaRepo.pushUnSyncedRecords() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
