...
class FilariaMDAFormSyncWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val preferenceDao: PreferenceDao = mockk(relaxed = true)
    private val repository: FilariaMDAFormRepository = mockk(relaxed = true)
    ...
    private fun worker(attempt: Int = 0): FilariaMDAFormSyncWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return FilariaMDAFormSyncWorker(context, params, preferenceDao, repository)
    }
    ...
    private fun sampleForm(id: Int = 1, hhId: Long = 5L) = FilariaMDAFormResponseJsonEntity(
        id = id,
        hhId = hhId,
    ...
    fun doSyncWork_marksFormSynced_whenPushSucceeds() = runTest {
        ...
        coEvery { repository.getUnsyncedForms(FormConstants.MDA_DISTRIBUTION_FORM_ID) } returns listOf(form)
    ...
    fun doSyncWork_savesVisits_whenResponseSuccessful() = runTest {
        ...
        assertTrue(result is ListenableWorker.Result.Success)
        coVerify { repository.saveDownloadedVisitList(visitList, FormConstants.MDA_DISTRIBUTION_FORM_ID) }
    }
