...
class MaaMeetingDownsyncWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val maaMeetingRepo: MaaMeetingRepo = mockk(relaxed = true)

    private fun worker(attempt: Int = 0): MaaMeetingDownsyncWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return MaaMeetingDownsyncWorker(context, params, maaMeetingRepo)
    }
