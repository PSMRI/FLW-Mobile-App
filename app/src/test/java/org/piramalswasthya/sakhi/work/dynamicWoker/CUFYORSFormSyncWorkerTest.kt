...
class CUFYORSFormSyncWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): CUFYORSFormSyncWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return CUFYORSFormSyncWorker(context, params, preferenceDao, repository)
    }
    ...
    fun doSyncWork_savesVisits_whenResponseSuccessful() = runTest {
        ...
        coEvery { repository.getAllFormVisits(FormConstants.ORS_FORM_NAME, any()) } returns response
    ...
    @Test
    fun doSyncWork_skipsSave_whenVisitListEmpty() = runTest {
        val response = mockk<Response<HBNCVisitListResponse>>()
        every { response.isSuccessful } returns true
        every { response.body() } returns HBNCVisitListResponse(data = emptyList())
