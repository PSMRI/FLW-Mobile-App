...
class PushTBToAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushTBToAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushTBToAmritWorker(context, params, tbRepo, preferenceDao)
    }
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { tbRepo.pushUnSyncedRecords() } throws java.net.SocketTimeoutException("timeout")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { tbRepo.pushUnSyncedRecords() } throws java.io.IOException("offline")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_returnsFailureWithMessage_whenGenericExceptionThrown() = runTest {
        coEvery { tbRepo.pushUnSyncedRecords() } throws IllegalStateException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionHasNoMessage() = runTest {
        coEvery { tbRepo.pushUnSyncedRecords() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
