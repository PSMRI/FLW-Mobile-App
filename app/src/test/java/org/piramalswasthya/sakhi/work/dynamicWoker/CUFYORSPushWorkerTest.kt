...
class CUFYORSPushWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): CUFYORSPushWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return CUFYORSPushWorker(context, params, preferenceDao, repository)
    }
    ...
    fun doSyncWork_marksFormSynced_whenPushSucceeds() = runTest {
        ...
        coEvery { repository.getUnsyncedForms(FormConstants.CHILDREN_UNDER_FIVE_ORS_FORM_ID) } returns listOf(form)
    ...
    @Test
    fun doSyncWork_returnsSuccess_whenPushThrows() = runTest {
