...
class PushVLFToAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushVLFToAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushVLFToAmritWorker(context, params, vlfRepo, preferenceDao)
    }
    ...
    @Test
    fun doWork_returnsSuccess_whenPushUnSyncedRecordsSucceeds() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } returns true
        val result = worker().doWork()
        assertTrue(result is ListenableWorker.Result.Success)
    }

    @Test
    fun doWork_returnsFailureWithSyncFalseMessage_whenPushUnSyncedRecordsFails() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } returns false
        val result = worker().doWork() as ListenableWorker.Result.Failure
        assertEquals("PushVLFToAmritWorker", result.outputData.getString("worker_name"))
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } throws SocketTimeoutException("timeout")
        val result = worker().doWork()
    ...
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } throws IOException("network down")
        val result = worker().doWork()
        assertTrue(result is ListenableWorker.Result.Retry)
    }
    ...
    fun doWork_returnsFailureWithExceptionMessage_whenGenericExceptionThrown() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } throws RuntimeException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
        assertEquals("PushVLFToAmritWorker", result.outputData.getString("worker_name"))
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionMessageIsNull() = runTest {
        coEvery { vlfRepo.pushUnSyncedRecords() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
