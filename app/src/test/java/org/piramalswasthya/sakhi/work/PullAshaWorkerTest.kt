...
class PullAshaWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val ashaRepo: AshaRepo = mockk(relaxed = true)
    ...
    private fun worker(attempt: Int = 0): PullAshaWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PullAshaWorker(context, params, ashaRepo, preferenceDao)
    }
