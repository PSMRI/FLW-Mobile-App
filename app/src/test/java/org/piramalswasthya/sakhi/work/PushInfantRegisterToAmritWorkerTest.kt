...
class PushInfantRegisterToAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushInfantRegisterToAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushInfantRegisterToAmritWorker(context, params, infantRegRepo, preferenceDao)
    }
    ...
    fun doSyncWork_returnsSuccess_whenRepoReturnsTrue() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } returns true
        assertTrue(worker().doWork() is ListenableWorker.Result.Success)
    }

    @Test
    fun doSyncWork_reportsSyncFalse_whenRepoReturnsFalse() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } returns false
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } throws java.net.SocketTimeoutException("timeout")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } throws java.io.IOException("offline")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_returnsFailureWithMessage_whenGenericExceptionThrown() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } throws IllegalStateException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionHasNoMessage() = runTest {
        coEvery { infantRegRepo.processNewInfantRegister() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
