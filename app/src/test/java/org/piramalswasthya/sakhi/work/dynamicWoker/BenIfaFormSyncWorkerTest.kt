...
class BenIfaFormSyncWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val preferenceDao: PreferenceDao = mockk(relaxed = true)
    private val repository: BenIfaFormRepository = mockk(relaxed = true)
    ...
    private fun worker(attempt: Int = 0): BenIfaFormSyncWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return BenIfaFormSyncWorker(context, params, preferenceDao, repository)
    }
    ...
    private fun sampleForm(id: Int = 1, benId: Long = 5L) = BenIfaFormResponseJsonEntity(
        id = id,
        benId = benId,
        hhId = 1L,
    ...
    fun doSyncWork_marksFormSynced_whenPushSucceeds() = runTest {
        ...
        coEvery { repository.getUnsyncedForms(FormConstants.IFA_DISTRIBUTION_FORM_ID) } returns listOf(form)
    ...
    fun doSyncWork_savesVisits_whenResponseSuccessful() = runTest {
        ...
        assertTrue(result is ListenableWorker.Result.Success)
        coVerify { repository.saveDownloadedVisitList(visitList, FormConstants.IFA_DISTRIBUTION_FORM_ID) }
    }
