...
class ReferPullFromAmritWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val referalRepo: NcdReferalRepo = mockk(relaxed = true)

    private fun worker(attempt: Int = 0): ReferPullFromAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return ReferPullFromAmritWorker(context, params, referalRepo)
    }
