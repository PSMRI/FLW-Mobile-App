...
class PullFilariaFromAmritWorkerTest : BaseRepositoryTest() {

    private val context: Context = mockk(relaxed = true)
    private val filariaRepo: FilariaRepo = mockk(relaxed = true)
    ...
    private fun worker(attempt: Int = 0): PullFilariaFromAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PullFilariaFromAmritWorker(context, params, filariaRepo, preferenceDao)
    }
    ...
    fun doWork_returnsSuccess_whenServerReturnsZero() = runTest {
        coEvery { filariaRepo.getFilariaScreeningDetailsFromServer() } returns 0
    ...
    fun doWork_returnsSuccess_whenServerReturnsOne() = runTest {
        coEvery { filariaRepo.getFilariaScreeningDetailsFromServer() } returns 1
        val result = worker().doWork()
        assertTrue(result is ListenableWorker.Result.Success)
    }
    ...
    fun doWork_returnsFailure_whenServerReturnsUnexpectedValue() = runTest {
        coEvery { filariaRepo.getFilariaScreeningDetailsFromServer() } returns 5
    ...
    fun doWork_returnsSuccess_whenRepoThrowsException_becauseInnerFailureIsSwallowed() = runTest {
        coEvery { filariaRepo.getFilariaScreeningDetailsFromServer() } throws RuntimeException("network down")
        val result = worker().doWork()
        assertTrue(result is ListenableWorker.Result.Success)
    }

    @Test
    fun doWork_returnsFailure_whenPreferenceDaoThrowsSqliteConstraintException() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns Konstants.defaultTimeStamp
        every { preferenceDao.getFirstSyncLastSyncedPage() } throws SQLiteConstraintException("constraint failed")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    @Test
    fun doWork_usesFirstSyncLastSyncedPage_whenTimestampIsDefault() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns Konstants.defaultTimeStamp
    ...
    @Test
    fun doWork_skipsFirstSyncLastSyncedPage_whenTimestampIsNotDefault() = runTest {
