...
class PushChildImmunizationToAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PushChildImmunizationToAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PushChildImmunizationToAmritWorker(context, params, immunizationRepo, preferenceDao)
    }
    ...
    fun doSyncWork_returnsSuccess_whenRepoReturnsTrue() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } returns true
        assertTrue(worker().doWork() is ListenableWorker.Result.Success)
    }

    @Test
    fun doSyncWork_reportsSyncFalse_whenRepoReturnsFalse() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } returns false
    ...
    fun doWork_retries_whenSocketTimeoutExceptionThrown() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } throws java.net.SocketTimeoutException("timeout")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_retries_whenIOExceptionThrown() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } throws java.io.IOException("offline")
        assertTrue(worker().doWork() is ListenableWorker.Result.Retry)
    }

    @Test
    fun doWork_returnsFailureWithMessage_whenGenericExceptionThrown() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } throws IllegalStateException("boom")
        val result = worker().doWork() as ListenableWorker.Result.Failure
    ...
    fun doWork_returnsFailureWithUnknownError_whenExceptionHasNoMessage() = runTest {
        coEvery { immunizationRepo.pushUnSyncedChildImmunizationRecords() } throws RuntimeException()
        val result = worker().doWork() as ListenableWorker.Result.Failure
