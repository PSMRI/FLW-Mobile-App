...
class CUFYSAMPushWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): CUFYSAMPushWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return CUFYSAMPushWorker(context, params, preferenceDao, repository)
    }
    ...
    fun doSyncWork_marksFormSynced_whenPushSucceeds() = runTest {
        ...
        coEvery { repository.getUnsyncedForms(FormConstants.CHILDREN_UNDER_FIVE_SAM_FORM_ID) } returns listOf(form)
