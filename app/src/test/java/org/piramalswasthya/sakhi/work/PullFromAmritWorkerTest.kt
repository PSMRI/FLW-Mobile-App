...
class PullFromAmritWorkerTest : BaseRepositoryTest() {
    ...
    private fun worker(attempt: Int = 0): PullFromAmritWorker {
        val params = mockk<WorkerParameters>(relaxed = true)
        every { params.runAttemptCount } returns attempt
        return PullFromAmritWorker(context, params, benRepo, preferenceDao)
    }
    ...
    fun doWork_returnsSuccess_whenNoPagesToSync() = runTest {
        ...
        coEvery { benRepo.getBeneficiariesFromServerForWorker(any()) } returns 0
    ...
    fun doWork_usesFirstSyncLastSyncedPage_whenLastSyncedTimestampIsDefault() = runTest {
        ...
        coEvery { benRepo.getBeneficiariesFromServerForWorker(2) } returns 0

        val result = worker().doWork()

        assertTrue(result is ListenableWorker.Result.Success)
        coVerify(exactly = 1) { benRepo.getBeneficiariesFromServerForWorker(2) }
    }

    @Test
    fun doWork_returnsSuccess_whenInitialFetchRetriesThenReturnsNoPages() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns 0L
        coEvery { benRepo.getBeneficiariesFromServerForWorker(0) } returnsMany listOf(-2, 0)

        val result = worker().doWork()

        assertTrue(result is ListenableWorker.Result.Success)
        coVerify(exactly = 2) { benRepo.getBeneficiariesFromServerForWorker(0) }
    }

    @Test
    fun doWork_returnsSuccess_whenAllPagesSyncSuccessfully() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns 0L
        coEvery { benRepo.getBeneficiariesFromServerForWorker(any()) } returns 1
    ...
    @Test
    fun doWork_returnsFailure_whenBenRepoReturnsMinusOne() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns 0L
        coEvery { benRepo.getBeneficiariesFromServerForWorker(any()) } returns 5
        coEvery { benRepo.getBeneficiariesFromServerForWorker(1) } returns -1

        val result = worker().doWork() as ListenableWorker.Result.Failure

        assertEquals("PullFromAmritWorker", result.outputData.getString("worker_name"))
    ...
    fun doWork_returnsSuccess_whenSQLiteConstraintExceptionCaughtInternally() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns 0L
        coEvery { benRepo.getBeneficiariesFromServerForWorker(any()) } returns 1
        coEvery { benRepo.getBeneficiariesFromServerForWorker(1) } throws SQLiteConstraintException("constraint violation")
    ...
    fun doWork_returnsFailure_whenSQLiteConstraintExceptionThrownDuringInitialFetch() = runTest {
        every { preferenceDao.getLastSyncedTimeStamp() } returns 0L
        coEvery { benRepo.getBeneficiariesFromServerForWorker(0) } throws SQLiteConstraintException("initial fetch constraint failure")
