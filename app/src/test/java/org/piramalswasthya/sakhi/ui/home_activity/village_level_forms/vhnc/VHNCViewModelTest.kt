...
class VHNCViewModelTest : BaseViewModelTest() {
    ...
    private lateinit var viewModel: VHNCViewModel
    ...
    override fun setUp() {
        ...
        viewModel = VHNCViewModel(savedStateHandle, preferenceDao, context, vlfRepo)
    }

    @Test fun `viewModel initializes successfully`() { assertNotNull(viewModel) }
    @Test fun `initial state is IDLE`() { assertEquals(VHNCViewModel.State.IDLE, viewModel.state.value) }
    @Test fun `resetState sets state to IDLE`() { viewModel.resetState(); assertEquals(VHNCViewModel.State.IDLE, viewModel.state.value) }
