...
class AHDFormFragmentNavigationTest {
    ...
    private fun args() = AHDFormFragmentArgs(id = 11)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("AHDFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), AHDFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = AHDFormFragmentArgs.fromSavedStateHandle(handle)
        assertEquals(0, a.id)
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), AHDFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = AHDFormFragmentArgs.fromBundle(bundle)
        assertEquals(0, a.id)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = AHDFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = AHDFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
