...
class HRPNonPregnantAssessFragmentNavigationTest {
    ...
    private fun args() = HRPNonPregnantAssessFragmentArgs(benId = 10L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("HRPNonPregnantAssessFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), HRPNonPregnantAssessFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantAssessFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantAssessFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), HRPNonPregnantAssessFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantAssessFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    @Test
    fun actionHRPNonPregnantAssessFragmentToNonPregnantListFragment_buildsDirections() {
        val d = HRPNonPregnantAssessFragmentDirections.actionHRPNonPregnantAssessFragmentToNonPregnantListFragment()
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HRPNonPregnantAssessFragmentDirections.actionHRPNonPregnantAssessFragmentToNonPregnantListFragment()
        assertEquals(d, same)
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = HRPNonPregnantAssessFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HRPNonPregnantAssessFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
        assertEquals(d.hashCode(), same.hashCode())
        assertFalse(d.equals(null))
        assertFalse(d.equals(Any()))
