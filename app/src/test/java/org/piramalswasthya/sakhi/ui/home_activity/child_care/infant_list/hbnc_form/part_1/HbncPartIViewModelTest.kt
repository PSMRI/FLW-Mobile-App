...
/**
 * Unit tests for [HbncPartIViewModel].
 *
 * `Dispatchers.IO` is redirected onto the test scheduler so the `init` block's
 * `withContext(Dispatchers.IO)` lookup runs deterministically under `advanceUntilIdle()`.
 * The real [org.piramalswasthya.sakhi.configuration.HBNCFormDataset] is built with mocked
 * resources exactly as the dataset test suite does.
 */
@OptIn(ExperimentalCoroutinesApi::class)
class HbncPartIViewModelTest : BaseViewModelTest() {
    ...
    override fun setUp() {
        ...
        every { preferenceDao.getLocationRecord() } returns mockk(relaxed = true)
        ...
        coEvery { hbncRepo.getHbncCard(any(), any()) } returns null
        coEvery { hbncRepo.saveHbncData(any()) } returns true
    }

    private fun buildVm(hhId: Long = 1L, benId: Long = 2L): HbncPartIViewModel =
        HbncPartIViewModel(
            context,
    ...
    fun `viewModel initializes with an idle state and a form list`() {
        ...
        assertEquals(HbncPartIViewModel.State.IDLE, vm.state.value)
    }

    @Test
    fun `init populates the beneficiary header when no record exists`() = runTest {
        val vm = buildVm()
        advanceUntilIdle()
    ...
    fun `init omits a null last name from the header`() = runTest {
        ...
        assertEquals("Baby ", vm.benName.value)
    }

    @Test
    fun `submitForm reports success when the repository saves the record`() = runTest {
        val vm = buildVm(hhId = 5L, benId = 6L)
        advanceUntilIdle()

        vm.submitForm()
        advanceUntilIdle()

        assertEquals(HbncPartIViewModel.State.SUCCESS, vm.state.value)
    ...
    fun `submitForm reports failure when the repository rejects the record`() = runTest {
        ...
        assertEquals(HbncPartIViewModel.State.FAIL, vm.state.value)
    }
    ...
    @Test
    fun `resetErrorMessage clears the dataset alert`() = runTest {
        val vm = buildVm()
        advanceUntilIdle()
