...
class HBNCFormFragmentNavigationTest {
    ...
    private fun args() = HBNCFormFragmentArgs(benId = 10L, hhId = 20L, visitDay = "v3", isViewMode = true, formId = "v5")
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("HBNCFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), HBNCFormFragmentArgs.fromSavedStateHandle(handle))
    }
    ...
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = HBNCFormFragmentArgs.fromSavedStateHandle(handle)
        assertNull(a.visitDay)
        assertEquals(false, a.isViewMode)
        assertEquals("hbnc_form_001", a.formId)
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            HBNCFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            HBNCFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), HBNCFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        every { bundle.getLong("benId") } returns 10L
        every { bundle.getLong("hhId") } returns 20L
        val a = HBNCFormFragmentArgs.fromBundle(bundle)
        assertNull(a.visitDay)
        assertEquals(false, a.isViewMode)
        assertEquals("hbnc_form_001", a.formId)
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            HBNCFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    @Test
    fun actionHbncFormFragmentToCdrObjectFragment_buildsDirections() {
        val d = HBNCFormFragmentDirections.actionHbncFormFragmentToCdrObjectFragment(hhId = 10L, benId = 20L)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HBNCFormFragmentDirections.actionHbncFormFragmentToCdrObjectFragment(hhId = 10L, benId = 20L)
        assertEquals(d, same)
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = HBNCFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HBNCFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
        assertEquals(d.hashCode(), same.hashCode())
        assertFalse(d.equals(null))
        assertFalse(d.equals(Any()))
    ...
    fun constructor_usesDefaults_whenOptionalArgumentsOmitted() {
        val a = HBNCFormFragmentArgs(benId = 10L, hhId = 20L)
        assertNull(a.visitDay)
