...
class EligibleCoupleTrackingFormFragmentNavigationTest {
    ...
    private fun args() = EligibleCoupleTrackingFormFragmentArgs(benId = 10L, createdDate = 20L)

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(10L, a.benId)
        assertEquals(20L, a.createdDate)
    }
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("EligibleCoupleTrackingFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(20L, handle.get<Long>("createdDate"))
        assertEquals(args(), EligibleCoupleTrackingFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to 10L))
        val a = EligibleCoupleTrackingFormFragmentArgs.fromSavedStateHandle(handle)
        assertEquals(0L, a.createdDate)
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            EligibleCoupleTrackingFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to null, "createdDate" to 20L))
        assertThrows(IllegalArgumentException::class.java) {
            EligibleCoupleTrackingFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getLong("createdDate") } returns 20L
        assertEquals(args(), EligibleCoupleTrackingFormFragmentArgs.fromBundle(bundle))
    }

    @Test
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        every { bundle.containsKey(any()) } returns true
        every { bundle.containsKey("createdDate") } returns false
        every { bundle.getLong("benId") } returns 10L
        val a = EligibleCoupleTrackingFormFragmentArgs.fromBundle(bundle)
        assertEquals(0L, a.createdDate)
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            EligibleCoupleTrackingFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = EligibleCoupleTrackingFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = EligibleCoupleTrackingFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
