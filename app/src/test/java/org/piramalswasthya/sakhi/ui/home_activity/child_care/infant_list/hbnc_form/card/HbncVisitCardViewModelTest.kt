...
/**
 * Unit tests for [HbncVisitCardViewModel]. The `init` block seeds the village/block/asha names on
 * the dataset only when no HBNC record exists yet, so both arms are covered here.
 */
@OptIn(ExperimentalCoroutinesApi::class)
class HbncVisitCardViewModelTest : BaseViewModelTest() {
    ...
    private fun buildVm(hhId: Long = 1L, benId: Long = 2L): HbncVisitCardViewModel =
        HbncVisitCardViewModel(
    ...
    fun `viewModel initializes with an idle state and a form list`() {
        ...
        assertEquals(HbncVisitCardViewModel.State.IDLE, vm.state.value)
    }

    @Test
    fun `init seeds location and asha details when no record exists`() = runTest {
        val vm = buildVm()
        advanceUntilIdle()
    ...
    @Test
    fun `init tolerates a missing location record`() = runTest {
        every { preferenceDao.getLocationRecord() } returns null
    ...
    @Test
    fun `init skips seeding when a record already exists`() = runTest {
    ...
    fun `submitForm reports success when the repository saves the record`() = runTest {
        ...
        assertEquals(HbncVisitCardViewModel.State.SUCCESS, vm.state.value)
    }
    ...
    fun `submitForm reports failure when the repository rejects the record`() = runTest {
        ...
        assertEquals(HbncVisitCardViewModel.State.FAIL, vm.state.value)
    }
    ...
    fun `updateListOnValueChanged delegates to the dataset`() = runTest {
        ...
        vm.updateListOnValueChanged(2, 1)
        advanceUntilIdle()
