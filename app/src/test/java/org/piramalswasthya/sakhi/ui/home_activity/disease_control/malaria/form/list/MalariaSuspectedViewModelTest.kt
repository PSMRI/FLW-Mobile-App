...
class MalariaSuspectedViewModelTest : BaseViewModelTest() {

    @MockK private lateinit var recordsRepo: RecordsRepo

    private lateinit var viewModel: MalariaSuspectedViewModel
    ...
    override fun setUp() {
        super.setUp()
        every { recordsRepo.malariaScreeningList(any()) } returns flowOf(emptyList())
        viewModel = MalariaSuspectedViewModel(recordsRepo, savedStateHandle)
    }
    ...
    fun `diseaseType is set from SavedStateHandle`() {
        assertEquals("MALARIA", viewModel.diseaseType)
    }
