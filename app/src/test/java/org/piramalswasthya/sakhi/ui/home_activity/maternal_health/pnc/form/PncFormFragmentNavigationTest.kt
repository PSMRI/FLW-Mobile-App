...
class PncFormFragmentNavigationTest {
    ...
    private fun args() = PncFormFragmentArgs(benId = 10L, hhId = "v2", visitNumber = 31)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("PncFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), PncFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            PncFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to null, "hhId" to "v2", "visitNumber" to 31))
        assertThrows(IllegalArgumentException::class.java) {
            PncFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), PncFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            PncFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    @Test
    fun actionPncFormFragmentToMdsrObjectFragment_buildsDirections() {
        val d = PncFormFragmentDirections.actionPncFormFragmentToMdsrObjectFragment(hhId = 10L, benId = 20L)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = PncFormFragmentDirections.actionPncFormFragmentToMdsrObjectFragment(hhId = 10L, benId = 20L)
        assertEquals(d, same)
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = PncFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = PncFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
        assertEquals(d.hashCode(), same.hashCode())
        assertFalse(d.equals(null))
        assertFalse(d.equals(Any()))
