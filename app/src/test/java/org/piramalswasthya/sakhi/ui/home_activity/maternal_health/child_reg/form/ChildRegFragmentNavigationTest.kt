...
class ChildRegFragmentNavigationTest {
    ...
    private fun args() = ChildRegFragmentArgs(motherBenId = 10L, babyIndex = 21)

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(10L, a.motherBenId)
        assertEquals(21, a.babyIndex)
    }
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("ChildRegFragmentArgs"))
    ...
    fun copy_replacesSingleArgument() {
        val a = args()
        val b = a.copy(motherBenId = 99L)
        assertEquals(99L, b.motherBenId)
        assertNotEquals(a, b)
    }
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(10L, handle.get<Long>("motherBenId"))
        assertEquals(21, handle.get<Int>("babyIndex"))
        assertEquals(args(), ChildRegFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            ChildRegFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("motherBenId" to null, "babyIndex" to 21))
        assertThrows(IllegalArgumentException::class.java) {
            ChildRegFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getLong("motherBenId") } returns 10L
        every { bundle.getInt("babyIndex") } returns 21
        assertEquals(args(), ChildRegFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            ChildRegFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = ChildRegFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = ChildRegFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
