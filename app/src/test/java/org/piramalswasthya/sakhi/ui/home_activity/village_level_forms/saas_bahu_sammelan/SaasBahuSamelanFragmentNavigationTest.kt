...
class SaasBahuSamelanFragmentNavigationTest {
    ...
    private fun args() = SaasBahuSamelanFragmentArgs(id = 10L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("SaasBahuSamelanFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(10L, handle.get<Long>("id"))
        assertEquals(args(), SaasBahuSamelanFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = SaasBahuSamelanFragmentArgs.fromSavedStateHandle(handle)
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getLong("id") } returns 10L
        assertEquals(args(), SaasBahuSamelanFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = SaasBahuSamelanFragmentArgs.fromBundle(bundle)
        assertEquals(0L, a.id)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = SaasBahuSamelanFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = SaasBahuSamelanFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
