...
class PregnancyRegistrationFormFragmentNavigationTest {
    ...
    private fun args() = PregnancyRegistrationFormFragmentArgs(benId = 10L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("PregnancyRegistrationFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), PregnancyRegistrationFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            PregnancyRegistrationFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            PregnancyRegistrationFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), PregnancyRegistrationFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            PregnancyRegistrationFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = PregnancyRegistrationFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = PregnancyRegistrationFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
