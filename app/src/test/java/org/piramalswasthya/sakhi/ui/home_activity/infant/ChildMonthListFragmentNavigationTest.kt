...
class ChildMonthListFragmentNavigationTest {
    ...
    private fun args() = ChildMonthListFragmentArgs(hhId = 10L, benId = 20L, dob = 30L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("ChildMonthListFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(30L, handle.get<Long>("dob"))
        assertEquals(args(), ChildMonthListFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            ChildMonthListFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("hhId" to null, "benId" to 20L, "dob" to 30L))
        assertThrows(IllegalArgumentException::class.java) {
            ChildMonthListFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getLong("dob") } returns 30L
        assertEquals(args(), ChildMonthListFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            ChildMonthListFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    @Test
    fun actionInfantFormFragmentToHBYCFormFragment_buildsDirections() {
        val d = ChildMonthListFragmentDirections.actionInfantFormFragmentToHBYCFormFragment(benId = 10L, hhId = 20L, visitDay = "v3", isViewMode = true, formId = "v5")
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = ChildMonthListFragmentDirections.actionInfantFormFragmentToHBYCFormFragment(benId = 10L, hhId = 20L, visitDay = "v3", isViewMode = true, formId = "v5")
        assertEquals(d, same)
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = ChildMonthListFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = ChildMonthListFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
        assertEquals(d.hashCode(), same.hashCode())
        assertFalse(d.equals(null))
        assertFalse(d.equals(Any()))
