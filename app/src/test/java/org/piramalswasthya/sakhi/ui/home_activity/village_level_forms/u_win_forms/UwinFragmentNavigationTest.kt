...
class UwinFragmentNavigationTest {
    ...
    private fun args() = UwinFragmentArgs(uwinId = 11)

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(11, a.uwinId)
    }
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("UwinFragmentArgs"))
    ...
    fun copy_replacesSingleArgument() {
        val a = args()
        val b = a.copy(uwinId = 99)
        assertEquals(99, b.uwinId)
        assertNotEquals(a, b)
    }
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(11, handle.get<Int>("uwinId"))
        assertEquals(args(), UwinFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = UwinFragmentArgs.fromSavedStateHandle(handle)
        assertEquals(0, a.uwinId)
    }

    @Test
    fun fromBundle_readsEveryArgument() {
        every { bundle.containsKey(any()) } returns true
        every { bundle.getInt("uwinId") } returns 11
        assertEquals(args(), UwinFragmentArgs.fromBundle(bundle))
    }

    @Test
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        every { bundle.containsKey(any()) } returns true
        every { bundle.containsKey("uwinId") } returns false
        val a = UwinFragmentArgs.fromBundle(bundle)
        assertEquals(0, a.uwinId)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = UwinFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = UwinFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
