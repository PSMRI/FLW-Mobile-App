...
class VHNCFormFragementNavigationTest {
    ...
    private fun args() = VHNCFormFragementArgs(id = 11)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("VHNCFormFragementArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), VHNCFormFragementArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = VHNCFormFragementArgs.fromSavedStateHandle(handle)
        assertEquals(0, a.id)
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), VHNCFormFragementArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = VHNCFormFragementArgs.fromBundle(bundle)
        assertEquals(0, a.id)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = VHNCFormFragementDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = VHNCFormFragementDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
