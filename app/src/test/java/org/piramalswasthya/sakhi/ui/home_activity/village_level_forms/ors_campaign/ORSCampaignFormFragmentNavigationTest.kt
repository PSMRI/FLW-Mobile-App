...
class ORSCampaignFormFragmentNavigationTest {
    ...
    private fun args() = ORSCampaignFormFragmentArgs(id = 11)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("ORSCampaignFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), ORSCampaignFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = ORSCampaignFormFragmentArgs.fromSavedStateHandle(handle)
        assertEquals(0, a.id)
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), ORSCampaignFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = ORSCampaignFormFragmentArgs.fromBundle(bundle)
        assertEquals(0, a.id)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = ORSCampaignFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = ORSCampaignFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
