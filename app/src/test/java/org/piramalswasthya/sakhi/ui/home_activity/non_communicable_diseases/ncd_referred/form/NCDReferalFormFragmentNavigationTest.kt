...
class NCDReferalFormFragmentNavigationTest {
    ...
    private fun args() = NCDReferalFormFragmentArgs(benId = 10L, referral = "v2", referralType = "v3", cbacId = 41)

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(10L, a.benId)
        assertEquals("v2", a.referral)
        assertEquals("v3", a.referralType)
        assertEquals(41, a.cbacId)
    }
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals("v2", handle.get<String>("referral"))
        assertEquals("v3", handle.get<String>("referralType"))
        assertEquals(41, handle.get<Int>("cbacId"))
    ...
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to 10L, "referral" to "v2", "referralType" to "v3"))
        val a = NCDReferalFormFragmentArgs.fromSavedStateHandle(handle)
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            NCDReferalFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to null, "referral" to "v2", "referralType" to "v3", "cbacId" to 41))
        assertThrows(IllegalArgumentException::class.java) {
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getString("referral") } returns "v2"
        every { bundle.getString("referralType") } returns "v3"
        every { bundle.getInt("cbacId") } returns 41
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        every { bundle.getString("referral") } returns "v2"
        every { bundle.getString("referralType") } returns "v3"
        val a = NCDReferalFormFragmentArgs.fromBundle(bundle)
        assertEquals(0, a.cbacId)
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            NCDReferalFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = NCDReferalFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = NCDReferalFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
