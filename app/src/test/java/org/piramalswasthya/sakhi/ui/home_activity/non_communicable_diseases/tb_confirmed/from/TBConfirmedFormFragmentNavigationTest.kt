...
class TBConfirmedFormFragmentNavigationTest {
    ...
    private fun args() = TBConfirmedFormFragmentArgs(benId = 10L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("TBConfirmedFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), TBConfirmedFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            TBConfirmedFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            TBConfirmedFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), TBConfirmedFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            TBConfirmedFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = TBConfirmedFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = TBConfirmedFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
