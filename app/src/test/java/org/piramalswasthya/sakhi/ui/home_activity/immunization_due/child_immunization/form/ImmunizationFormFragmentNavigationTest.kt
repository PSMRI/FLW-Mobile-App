...
class ImmunizationFormFragmentNavigationTest {
    ...
    private fun args() = ImmunizationFormFragmentArgs(vaccineId = 11, benId = 20L, category = "v3")

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(11, a.vaccineId)
        assertEquals(20L, a.benId)
        assertEquals("v3", a.category)
    }
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("ImmunizationFormFragmentArgs"))
    ...
    fun copy_replacesSingleArgument() {
        val a = args()
        val b = a.copy(vaccineId = 99)
        assertEquals(99, b.vaccineId)
        assertNotEquals(a, b)
    }
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(11, handle.get<Int>("vaccineId"))
        assertEquals(20L, handle.get<Long>("benId"))
        assertEquals("v3", handle.get<String>("category"))
        assertEquals(args(), ImmunizationFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            ImmunizationFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("vaccineId" to null, "benId" to 20L, "category" to "v3"))
        assertThrows(IllegalArgumentException::class.java) {
            ImmunizationFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getInt("vaccineId") } returns 11
        every { bundle.getLong("benId") } returns 20L
        every { bundle.getString("category") } returns "v3"
        assertEquals(args(), ImmunizationFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            ImmunizationFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = ImmunizationFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = ImmunizationFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
