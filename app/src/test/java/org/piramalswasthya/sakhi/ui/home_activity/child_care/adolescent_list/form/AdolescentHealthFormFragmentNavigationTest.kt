...
class AdolescentHealthFormFragmentNavigationTest {
    ...
    private fun args() = AdolescentHealthFormFragmentArgs(benId = 10L)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("AdolescentHealthFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), AdolescentHealthFormFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = AdolescentHealthFormFragmentArgs.fromSavedStateHandle(handle)
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), AdolescentHealthFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = AdolescentHealthFormFragmentArgs.fromBundle(bundle)
        assertEquals(0L, a.benId)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = AdolescentHealthFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = AdolescentHealthFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
