...
class PHCReviewFormFragementNavigationTest {
    ...
    private fun args() = PHCReviewFormFragementArgs(id = 11)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("PHCReviewFormFragementArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), PHCReviewFormFragementArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        val handle = SavedStateHandle()
        val a = PHCReviewFormFragementArgs.fromSavedStateHandle(handle)
        assertEquals(0, a.id)
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), PHCReviewFormFragementArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = PHCReviewFormFragementArgs.fromBundle(bundle)
        assertEquals(0, a.id)
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = PHCReviewFormFragementDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = PHCReviewFormFragementDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
