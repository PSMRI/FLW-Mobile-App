...
class HRPNonPregnantTrackFragmentNavigationTest {
    ...
    private fun args() = HRPNonPregnantTrackFragmentArgs(benId = 10L, trackId = 21)

    @Test
    fun constructor_exposesEveryArgument() {
        val a = args()
        assertEquals(10L, a.benId)
        assertEquals(21, a.trackId)
    }
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("HRPNonPregnantTrackFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(21, handle.get<Int>("trackId"))
        assertEquals(args(), HRPNonPregnantTrackFragmentArgs.fromSavedStateHandle(handle))
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantTrackFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("benId" to null, "trackId" to 21))
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantTrackFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        every { bundle.getInt("trackId") } returns 21
        assertEquals(args(), HRPNonPregnantTrackFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            HRPNonPregnantTrackFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    @Test
    fun actionHRPNonPregnantTrackFragmentToHRPNonPregnantListFragment_buildsDirections() {
        val d = HRPNonPregnantTrackFragmentDirections.actionHRPNonPregnantTrackFragmentToHRPNonPregnantListFragment()
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HRPNonPregnantTrackFragmentDirections.actionHRPNonPregnantTrackFragmentToHRPNonPregnantListFragment()
        assertEquals(d, same)
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = HRPNonPregnantTrackFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = HRPNonPregnantTrackFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
        assertEquals(d.hashCode(), same.hashCode())
        assertFalse(d.equals(null))
        assertFalse(d.equals(Any()))
