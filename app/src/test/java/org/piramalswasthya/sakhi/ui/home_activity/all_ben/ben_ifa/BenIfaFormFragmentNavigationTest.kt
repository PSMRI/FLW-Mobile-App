...
class BenIfaFormFragmentNavigationTest {
    ...
    private fun args() = BenIfaFormFragmentArgs(hhId = 10L, benId = 20L, isViewMode = true)
    ...
    fun dataClassSynthetics_behaveAsValueType() {
        ...
        assertTrue(a.toString().contains("BenIfaFormFragmentArgs"))
    ...
    fun toSavedStateHandle_thenFromSavedStateHandle_roundTrips() {
        ...
        assertEquals(args(), BenIfaFormFragmentArgs.fromSavedStateHandle(handle))
    }
    ...
    fun fromSavedStateHandle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        val a = BenIfaFormFragmentArgs.fromSavedStateHandle(handle)
    ...
    fun fromSavedStateHandle_throws_whenRequiredArgumentMissing() {
        assertThrows(IllegalArgumentException::class.java) {
            BenIfaFormFragmentArgs.fromSavedStateHandle(SavedStateHandle())
        }
    }

    @Test
    fun fromSavedStateHandle_throws_whenRequiredArgumentIsNull() {
        val handle = SavedStateHandle(mapOf<String, Any?>("hhId" to null, "benId" to 20L, "isViewMode" to true))
        assertThrows(IllegalArgumentException::class.java) {
            BenIfaFormFragmentArgs.fromSavedStateHandle(handle)
        }
    }
    ...
    fun fromBundle_readsEveryArgument() {
        ...
        assertEquals(args(), BenIfaFormFragmentArgs.fromBundle(bundle))
    }
    ...
    fun fromBundle_appliesDefaults_whenOptionalArgumentsMissing() {
        ...
        every { bundle.getLong("hhId") } returns 10L
        val a = BenIfaFormFragmentArgs.fromBundle(bundle)
        assertEquals(0L, a.benId)
        assertEquals(false, a.isViewMode)
    }
    ...
    fun fromBundle_throws_whenRequiredArgumentMissing() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            BenIfaFormFragmentArgs.fromBundle(bundle)
        }
    }
    ...
    fun actionGlobalPwAncFormFragment_buildsDirections() {
        val d = BenIfaFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertNotNull(d)
        assertTrue(d.actionId != 0)
        assertNotNull(d.arguments)
        assertTrue(d.toString().isNotEmpty())
        val same = BenIfaFormFragmentDirections.actionGlobalPwAncFormFragment(benId = 10L, hhId = "v2", visitNumber = 31, fromPmsma = true, lastItemClick = true)
        assertEquals(d, same)
