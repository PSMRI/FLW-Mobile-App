...
class HRPPregnantAssessDatasetTest : BaseViewModelTest() {
    ...
    @Test
    fun hrpPregnantAssessDeep() = runTest {
        val ds = HRPPregnantAssessDataset(context, Languages.ENGLISH)
        val ben = mockk<BenRegCache>(relaxed = true)
        val saved = mockk<HRPPregnantAssessCache>(relaxed = true)
        runCatching { ds.setUpPage(null, null) }
        runCatching { ds.setUpPage(ben, saved) }
        runCatching { ds.mapValues(mockk<HRPPregnantAssessCache>(relaxed = true), 0) }
