...
class HBYCFormDatasetTest : BaseViewModelTest() {
    ...
    @Test
    fun hbycFormDeep() = runTest {
        val ds = HBYCFormDataset(context, Languages.ENGLISH)
        val ben = mockk<BenRegCache>(relaxed = true)
        val saved = mockk<HBYCCache>(relaxed = true)
        for (month in listOf("3", "6", "9", "12", "15")) {
            runCatching { ds.setUpPage(null, null, month) }
            runCatching { ds.setUpPage(ben, saved, month) }
        }
