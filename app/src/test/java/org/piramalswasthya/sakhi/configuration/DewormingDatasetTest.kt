...
/**
 * Deep coverage test for [DewormingDataset]: exercises setUpPage (create + cached),
 * mapValues, getFormElementList and setImageUriToFormElement. Each builder call
 * wrapped in runCatching.
 */
@OptIn(ExperimentalCoroutinesApi::class)
class DewormingDatasetTest : BaseViewModelTest() {
    ...
    fun `create path exercises builders`() = runTest {
        ...
        // setUpPage(deworming: DewormingCache?)
        runCatching { ds.setUpPage(null) }
        runCatching { ds.mapValues(mockk<DewormingCache>(relaxed = true), 0) }
        runCatching { ds.mapValues(mockk<DewormingCache>(relaxed = true), 1) }
